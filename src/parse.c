...
int mi_get_tuple(mi_results *r, const char *str, const char **end)
{
    ...
    r->type = t_tuple;
    str++;
    if (*str == '}') { /* Special case: empty tuple */
        *end = str + 1;
        return 1;
    }
#ifdef __APPLE__
...
int mi_get_list(mi_results *r, const char *str, const char **end)
{
    ...
    r->type = t_list;
    str++;
    if (*str == ']') { /* Special case: empty list */
        *end = str + 1;
        return 1;
    }
...
int mi_get_async_stop_reason(mi_output *r, char **reason)
{
    ...
    *reason = NULL;
    while (r) {
        ...
        if (r->type == MI_T_OUT_OF_BAND && r->stype == MI_ST_ASYNC && r->sstype == MI_SST_EXEC &&
            r->tclass == MI_CL_STOPPED) {
            mi_results *p = r->c;
            found_stopped = 1;
            while (p) {
...
int mi_get_thread_ids(mi_output *res, int **list)
{
    ...
    *list = NULL;
    ...
    if (vids && vids->type == t_const && lids && lids->type == t_tuple) {
...
mi_gvar *mi_get_gvar(mi_output *o, mi_gvar *cur, const char *expression)
{
    ...
    while (r) {
        if (r->type == t_const) {
            ...
            } else if (strcmp(r->var, "attr") == 0) { /* Note: gdb 6.1.1 have only this: */
...
int mi_res_changelist(mi_h *h, mi_gvar_chg **changed)
{
    ...
    *changed = NULL;
    ...
    if (res->type == t_list) { // MI v2 a list of tuples
        while (r) {
            if (r->type == t_tuple) {
    ...
    } else if (res->type == t_tuple) { // MI v1 a tuple with all together *8-P
        while (r) {
            if (r->type == t_const) /* Just in case. */
            {                       /* Get one var. */
                if (strcmp(r->var, "name") == 0) {
                    if (n) { /* Add to the list*/
                        if (last)
                            last->next = n;
                        else
                            *changed = n;
...
static char *reason_names[] = { "breakpoint-hit",          "watchpoint-trigger",
                                "read-watchpoint-trigger", "access-watchpoint-trigger",
                                "watchpoint-scope",        "function-finished",
                                "location-reached",        "end-stepping-range",
                                "exited-signalled",        "exited",
                                "exited-normally",         "signal-received" };

static enum mi_stop_reason reason_values[] = { sr_bkpt_hit,         sr_wp_trigger,
                                               sr_read_wp_trigger,  sr_access_wp_trigger,
                                               sr_wp_scope,         sr_function_finished,
                                               sr_location_reached, sr_end_stepping_range,
                                               sr_exited_signalled, sr_exited,
                                               sr_exited_normally,  sr_signal_received };

static char *reason_expl[] = { "Hit a breakpoint",  "Write watchpoint",        "Read watchpoint",
                               "Access watchpoint", "Watchpoint out of scope", "Function finished",
                               "Location reached",  "End of stepping",         "Exited signalled",
                               "Exited with error", "Exited normally",         "Signal received" };
    ...
    if (r && r->type == t_list && ws == 1) {
        ...
        while (r) {
            if (r->type == t_list && strcmp(r->var, "data") == 0) {
                ...
                ok++;
                if (data && data->type == t_const && strcmp(data->v.cstr, "N/A") == 0)
                    *na = 1;
                else
                    while (data) {
...
mi_asm_insns *mi_parse_insns(mi_results *c)
{
    ...
    while (c) {
        ...
        } else { /* No source line, just instructions */
