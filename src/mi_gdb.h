...
public:
    ...
    int Disconnect();
    /* SelectTarget* */
    int SelectTargetX11(const char *exec, const char *args = NULL, const char *auxtty = NULL);
    int SelectTargetLinux(const char *exec, const char *args, const char *auxtty = NULL);
    int SelectTargetRemote(const char *exec, const char *rparams, const char *rtype = NULL,
                           bool download = false);
    ...
    int ReadMemory(const char *exp, unsigned size, unsigned char *dest, int &na, int convAddr,
                   unsigned long *addr)
    {
        if (state != stopped)
            return 0;
    ...
    void SetConsoleCB(stream_cb cb, void *data = NULL) { mi_set_console_cb(h, cb, data); }
    void SetTargetCB(stream_cb cb, void *data = NULL) { mi_set_target_cb(h, cb, data); }
    void SetLogCB(stream_cb cb, void *data = NULL) { mi_set_log_cb(h, cb, data); }
    void SetAsyncCB(async_cb cb, void *data = NULL) { mi_set_async_cb(h, cb, data); }
    void SetToGDBCB(stream_cb cb, void *data = NULL) { mi_set_to_gdb_cb(h, cb, data); }
    void SetFromGDBCB(stream_cb cb, void *data = NULL) { mi_set_from_gdb_cb(h, cb, data); }
    void SetTimeOutCB(tm_cb cb, void *data) { mi_set_time_out_cb(h, cb, data); }
    void SetTimeOut(int to) { mi_set_time_out(h, to); }
    void ForceMIVersion(unsigned vMajor, unsigned vMiddle, unsigned vMinor)
    {
        mi_force_version(h, vMajor, vMiddle, vMinor);
    }

    const char *GetAuxTTY() { return aux_tty ? aux_tty->tty : NULL; }
...
protected:
    ...
    int SelectTargetTTY(const char *exec, const char *args, const char *auxtty, dMode m);
};
