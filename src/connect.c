...
void mi_free_h(mi_h **handle)
{
    ...
    if (h->from_gdb[1] >= 0)
        close(h->from_gdb[1]);
    if (mi_check_running(h)) { /* GDB is running! */
        mi_kill_child(h->pid);
    }
    if (h->line)
        free(h->line);
...
int mi_get_response(mi_h *h)
{
    ...
    if (strncmp(h->line, "(gdb)", 5) == 0) { /* End of response. */
        return 1;
    } else { /* Add to the response. */
        mi_output *o;
        int add = 1, is_exit = 0;
        ...
        } else if (o->type == MI_T_RESULT_RECORD &&
                   o->tclass == MI_CL_ERROR) { /* Error from gdb, record it. */
...
{
    if (!gdb_exe) { /* Look for gdb in path */
...
{
    if (!xterm_exe) { /* Look for xterm in path */
