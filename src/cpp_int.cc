...
int MIDebugger::SelectTargetTTY(const char *exec, const char *args, const char *auxtty, dMode m)
{
    if (state != connected)
        return 0;
...
int MIDebugger::SelectTargetX11(const char *exec, const char *args, const char *auxtty)
{
...
int MIDebugger::SelectTargetLinux(const char *exec, const char *args, const char *auxtty)
{
...
int MIDebugger::SelectTargetRemote(const char *exec, const char *rparams, const char *rtype,
                                   bool download)
{
    if (state != connected)
        return 0;
...
int MIDebugger::Poll(mi_stop *&rs)
{
    ...
    if (res) {
        if (res->reason == sr_exited_signalled || res->reason == sr_exited ||
    ...
    } else { // We got an error. It looks like most async commands returns running even
...
mi_bkpt *MIDebugger::Breakpoint(const char *where, bool temporary, const char *cond, int count,
                                int thread, bool hard_assist)
{
    if (state != stopped && state != target_specified)
        return NULL;
...
mi_bkpt *MIDebugger::Breakpoint(mi_bkpt *b)
{
    ...
    return Breakpoint(buf, b->disp == d_del, b->cond, b->ignore, b->thread, b->type == t_hw);
}
...
mi_bkpt *MIDebugger::BreakpointFull(const char *file, int line, bool temporary, const char *cond,
                                    int count, int thread, bool hard_assist)
{
    if (state != stopped && state != target_specified)
        return NULL;
    return gmi_break_insert_full_fl(h, file, line, temporary, hard_assist, cond, count, thread);
}
...
int MIDebugger::StepOver(bool inst)
{
    ...
    if (state == target_specified) { // We aren't running
        // Walk to main
        return RunToMain();
    }
    if (state == stopped) {
...
mi_frames *MIDebugger::CallStack(bool args)
{
    ...
    if (fr1 && args) { // Get the function arguments
        mi_frames *fr2 = gmi_stack_list_arguments(h, 1);
        if (fr2) { // Transfer them to the other list
...
{
    if (state == disconnected || state == running) // No async :-(
        return NULL;
    // Evaluate it
    mi_error  = MI_OK;
    char *res = gmi_data_evaluate_expression(h, exp);
    if (!res && mi_error_from_gdb) { // Not valid, return the error
